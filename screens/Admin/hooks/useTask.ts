...
const getMSSTableData = useCallback(async (page = currentPage, limit) => {
  setLoading(true);
  try {
    const res = await axios.get(`${BASE_URL}/task/mss?page=${page}&limit=${limit}`, {
      headers: {
...
const getMonthlyMissed = async () => {
  ...
  return {
    ...
    mssData,
    getMonthlyMissed,
    monthlyMissed, currentPage, setCurrentPage, totalPages,
  };
};
