...
export default function CleanerTasks({ route, navigation }) {
  ...
  useEffect(() => {
    const getTasksForLocation = async () => {
      try {
        const res = await axios.get(
          `https://sanitrack-node-api.onrender.com/api/cleaner-dashboard/rooms?locationId=${locationId}`,
          {
            headers: {
    ...
    if (locationId) { // Check if locationId is present
      getTasksForLocation();
    } else {
  ...
  return (
    ...
    <Text style={{fontSize: 30, marginTop: 20, fontWeight: 'bold', color: colors.white}}>Assigned Rooms</Text>
      {Array.isArray(tasks) && tasks.map((task, index) => (
        <TouchableOpacity
          key={index}
          style={styles.task}
          onPress={() => {
            navigation.navigate("CleanerRoom", { roomId: task.roomId, taskId: task.taskId }); 
          }}
        >
