...
const FacilityStopWatch = ({ startTime, setStartTime, endTime, setEndTime, stage ,id}) => {
  ...
  const filteringStage = facilityStages.filter(item => item?.name === stage);
...
console.log("sdfjfdjdjjd",stage)
  const startTimer = async () => {
   
    await axios
      .post(
        `${BASE_URL}inspector/facility-actual-time?work_order_id=${params.id}`,
        { stage_name: stage },
      ...
      .then(response => {
        ...
        if (response.data) {
          ...
          toast.success('Started Timer Successfully', {
            position: 'top-center',
            autoClose: 5000,
  ...
  const stopTimer = async () => {
    ...
    await axios
      .post(
        `${BASE_URL}inspector/facility-actual-stop?work_order_id=${params.id}&start_stage_id=${filteringStage[0]?._id}`,
        { stage_name: stage },
        { headers: { Authorization: `Bearer ${access_token}` } }
      )
      .then(response => {
        ...
        if (response.data) {
          getFacilityStages(params.id);
          toast.info('Ended Timer Successfully', {
            position: 'top-center',
            autoClose: 5000,
            hideProgressBar: true,
            closeOnClick: true,
      ...
      <div className="flex flex-col items-center justify-center">
        ...
        <div className="flex space-x-4">
          <span className="flex flex-col gap-y-2 items-center">
            <button
            disabled={filteringStage[0]?._id}
              onClick={handleStart}
              className={` p-3 disabled:bg-gray-300  text-white font-bold rounded-full ${
...
export default FacilityStopWatch;
