...
import CleanerLocationModal from './CleanerLocationModal';
import { useItemState } from 'context/ItemContext';

const CleanerLocationDetails = () => {
    const { setInventory } = useItemState();
  const { loading, roomDetails, getRoomDetailsForCleaner } = useCleanerHook();
  ...
  useEffect(() => {
    getRoomDetailsForCleaner(params.id);
  }, []);
  ...
  return (
    <>
      ...
      <ModalComponent isOpen={isModalOpen} onClose={closeModal} setIsModalOpen={setIsModalOpen}>
        <CleanerLocationModal closeModal={closeModal} />
      </ModalComponent>
    </>
  );
};

export default CleanerLocationDetails;
