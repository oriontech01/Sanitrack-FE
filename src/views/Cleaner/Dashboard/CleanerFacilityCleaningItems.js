...
const CleanerFacilityCleaningItems = () => {
  const { cleaningItems, setCleaningItems, getCleaningItemsForCleaner, loading,getRoomsToClean,rooms } = useCleanerHook();
  ...
  return (
    <>
      ...
      {cleaningItems?.length > 0 && <p className="text-sm text-gray-400 p-4">Items to Clean :{rooms?.length}</p>}
...
export default CleanerFacilityCleaningItems;
